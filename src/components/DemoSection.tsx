...
export default function DemoSection() {
  ...
  const startCall = useCallback(() => {
    ...
    // Phase 2: Connected (after 2s ring)
    schedulePhase(() => {
      setPhase("connected");
      setShowMsg1(true);
    }, 2000);

    // Phase 3: Talking - driver responds (after AI message types)
    schedulePhase(() => {
      setPhase("talking");
      setShowMsg2(true);
    }, 5500);

    // Phase 4: Processing - AI confirms
    schedulePhase(() => {
      setPhase("processing");
      setShowMsg3(true);
    }, 8500);

    // Phase 5: Done - card moves
    schedulePhase(() => {
      setPhase("done");
    ...
    <section id="demo" className="relative py-32 px-6 lg:px-8" ref={ref}>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div
          ...
          className="relative"
        >
          {/* Glow - pulsates when running */}
          <motion.div
            animate={
              isRunning
                ? { opacity: [0.06, 0.12, 0.06], scale: [1, 1.02, 1] }
                    ...
                    <AnimatePresence>
                      {phoneError && (
                        <motion.p
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: -4 }}
                          className="text-[11px] text-rose-300 mb-2.5"
                        >
                          {phoneError}
                        </motion.p>
                      )}
                  ...
                  <AnimatePresence>
                    {phase === "done" && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0 }}
                        className="flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-500/[0.06] border border-emerald-500/15 mb-5"
                      >
